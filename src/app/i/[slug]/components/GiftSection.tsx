...
export default function GiftSection({ bankAccounts }: { guestToken?: string; bankAccounts?: any[] }) {
  ...
  return (
    <section id="section-gift" className="relative py-20 md:py-32 px-6">
      <div className="max-w-lg mx-auto">
        <motion.div className="text-center mb-12" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <h2 className="section-title">Digital Gift</h2>
        ...
        <div className="space-y-4 mb-8">
          {banks.map((bank: any, i: number) => (
            <motion.div key={i} className="card flex items-center justify-between" initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}>
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-[#22382D]/5 flex items-center justify-center"><Building size={24} className="text-[#6F7F55]" /></div>
                <div>
              ...
              <motion.button onClick={() => copyAccount(bank.account_number, i)} className={`flex items-center gap-1.5 px-3 py-2 rounded-full text-xs font-medium ${copiedIndex === i ? 'bg-[#6F7F55] text-white' : 'bg-[#C9A86A]/10 text-[#C9A86A]'}`} whileTap={{ scale: 0.95 }}>
                {copiedIndex === i ? <Check size={14} /> : <Copy size={14} />}{copiedIndex === i ? 'Tersalin' : 'Salin'}
              </motion.button>
            </motion.div>
          ))}
        </div>
